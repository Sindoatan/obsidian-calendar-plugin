...
import type { TFile } from "obsidian";
  ...
  (granularity: IGranularity, date: Moment, existingFile: TFile, inNewSplit: boolean) => boolean,
  (granularity: IGranularity, date: Moment, existingFile: TFile, inNewSplit: boolean) => boolean,
  (granularity: IGranularity, date: Moment, file: TFile, event: MouseEvent) => boolean,
  (periodicity: IGranularity, date: Moment, file: TFile, targetEl: EventTarget, isMetaPressed: boolean) => boolean
];
        ...
        <tr>
          {#if showWeekNums}
            <WeekNum
              ...
              getSourceSettings="{getSourceSettings}"
              {...week}
              onClick={eventHandlers && eventHandlers[1]}
              onHover={eventHandlers && eventHandlers[3]}
              onContextMenu={eventHandlers && eventHandlers[2]}
              on:hoverDay="{updatePopover}"
              on:endHoverDay="{dismissPopover}"
            />
          {/if}
          {#each week.days as day (day.format())}
            <Day
              ...
              today="{today}"
              selectedId="{selectedId}"
              onClick={eventHandlers && eventHandlers[0]}
              onHover={eventHandlers && eventHandlers[3]}
              onContextMenu={eventHandlers && eventHandlers[2]}
              on:hoverDay="{updatePopover}"
              on:endHoverDay="{dismissPopover}"
